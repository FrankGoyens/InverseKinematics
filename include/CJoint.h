...
{
public:
	CJoint(const float minAngle, const float maxAngle, float offset, float angle, unsigned int childrenAmount, MoveHandler* handler, CSkeleton* skeleton);
	~CJoint();
	...
	void move(CJoint* joint, std::vector< JacobianElement > jacobian, glm::mat4 worldTransform, glm::vec3 newPosition, glm::vec3 forceVec);
