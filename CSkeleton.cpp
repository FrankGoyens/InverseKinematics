...
CJoint* CSkeleton::getJointFromString(string jointString, MoveHandler *handler)
{
	...
	float angle = getValueFromString(jointString, 0, spaces),
		lowerBound = getValueFromString(jointString, 1, spaces),
		upperBound = getValueFromString(jointString, 2, spaces),
		offset = getValueFromString(jointString, 3, spaces);
...
CLink* CSkeleton::getLinkFromString(string linkString)
{
	...
	float length = getValueFromString(linkString, 1, spaces),
			twist = getValueFromString(linkString, 2, spaces);
...
void CSkeleton::setPosition(glm::vec3 position)
{
	this->position = position;
}

void CSkeleton::setTransformMatrix(glm::mat4 matrix)
{
	vpMatrix = matrix;
}
